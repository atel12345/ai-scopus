...
def run_pipeline_job(job_id, scopus_link_or_id, author_name):
    try:
        ...
        _set_job(job_id, step="fetching_publications", current=2, message="Recuperation des publications")
        try:
        ...
        _set_job(
            ...
            current=3,
            message=f"{publication_count} publications trouvees",
            sub_progress=0,
        )
        ...
        for index, pub in enumerate(publications_raw, start=1):
            ...
            _set_job(
                ...
                current=3,
                message=f"{index}/{publication_count} publications traitees",
                sub_progress=index / publication_count if publication_count else 1,
            )

        _set_job(job_id, step="generating_excel", current=4, message="Generation du fichier Excel")
        excel_data = []
        ...
        _set_job(
            ...
            current=5,
            message="Rapport pret au telechargement",
            sub_progress=1,
            file_path=output_path,
            filename=output_filename,
        )
    except Exception as error:
        _set_job(job_id, status="error", error=str(error), message="La generation a echoue")
